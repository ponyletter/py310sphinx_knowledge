...
```text
开发者凭据区：
├── AppID (小程序ID)     : wxef6c0e98e63627a8 (公开可见，客户端配置用)
...
| :--- | :--- | :--- | :--- |
| **request 合法域名** | 支撑 `wx.request` 发起的数据请求（登录、资料、订单） | `https://apiwx.tg-cc755.cn` | ① **协议必须是 HTTPS**，不支持 HTTP；<br>② **不能写 IP 地址**（例如 `http://81.69.190.161:8280` 会直接报错）；<br>③ **不能带端口号**（只能默认 443 端口）；<br>④ 域名必须具有工信部 ICP 备案号。 |
